...
from cclib.tests.data.common import get_minimum_carbon_separation

from cclib.tests.data.skip import skipForParser, skipForLogfile
from cclib.tests.data.suite import DataSuite


class GenericSPTest(DataSuite):
    ...
    skip = {
        "feature": {
            ...
            "_atomcoords": [],
            "_atomcoords_units": [],
            "_charge_and_mult": ["Molcas"],
            # skipForParser('Molcas', 'missing mult')
            "_nbasis": [],
            "_atombasis": ["ADF", "Molcas", "Turbomole"],
            ...
            # skipForParser('Turbomole','The parser is still being developed so we skip this test')
            "_coreelectrons": ["Turbomole"],
            # skipForParser('Turbomole','The parser is still being developed so we skip this test')
            "_symlabels": ["FChk", "Molcas", "Molpro"],
    ...
    def test_SP(self):
        for self.data, self.logfile in zip(self.all_data, self.all_logfile):
            for feature in [
                ...
                if self.parsers in self.skip["feature"][feature]:
                    continue
                for c in self.skip["file"][feature]:
                    if self.logfile.startswith(c):
                        break
                else:
    ...
    def _symlabels(self):
        ...
        sumwronglabels = sum(
            [x not in ["Ag", "Bu", "Au", "Bg"] for x in self.data.mosyms[0]]
        )
    ...
    '''
    @skipForLogfile(
        "Gaussian/basicGaussian16/dvb_sp_no.out", "no energies for natural orbitals"
    )
    @skipForLogfile(
        "Turbomole/basicTurbomole5.9/dvb_sp_symm", "delta of 7.4, everything else ok"
    )
    def _firstmoenergy(self):
        """Is the lowest energy molecular orbital within the target?"""
        assert (
            abs(self.data.moenergies[0][0] - self.b3lyp_moenergy)
    ...
    @skipForParser("Molcas", "The parser is still being developed so we skip this test")
    @skipForParser(
        "Psi4", "Psi4 does not currently have the option to print the overlap matrix"
    )
    ...
    @skipForParser(
        "FChk", "Formatted checkpoint files do not have an input file section"
    )
    @skipForParser("GAMESS", "reading input file contents and name is not implemented")
    @skipForParser(
        "GAMESSUK", "reading input file contents and name is not implemented"
    )
    @skipForParser(
        "Gaussian", "reading input file contents and name is not implemented"
    )
    ...
    @skipForParser(
        "Turbomole", "reading input file contents and name is not implemented"
    )
    def _metadata_input_file(self):
        """Does metadata have expected keys and values?"""
    ...
    @skipForParser(
        "FChk",
        "Formatted Checkpoint files do not have section for legacy package version",
    )
    def _metadata_legacy_package_version(self):
        """Does metadata have expected keys and values?"""
        # TODO Test specific values for each unit test.
        assert "legacy_package_version" in self.data.metadata

    @skipForParser(
        "FChk", "Formatted Checkpoint files do not have section for package version"
    )
    def _metadata_package_version(self):
        """Does metadata have expected keys and values?"""
        # TODO Test specific values for each unit test.
        assert isinstance(
    ...
    @skipForParser(
        "Turbomole", "reading point group symmetry and name is not implemented"
    )
    def _metadata_symmetry_detected(self):
        """Does metadata have expected keys and values?"""
    ...
    def _metadata_symmetry_used(self):
        """Does metadata have expected keys and values?"""
    ...
    @skipForParser(
        "GAMESSUK", "reading cpu/wall time is not implemented for this parser"
    )
    @skipForParser(
        "GAMESSUS", "reading cpu/wall time is not implemented for this parser"
    )
    @skipForParser("Jaguar", "reading cpu/wall time is not implemented for this parser")
    @skipForParser(
        "Molcas", " reading cpu/wall time is not implemented for this parser"
    )
...
class TurbomoleSPTest(GenericSPTest):
    ...
    def _metadata_basis_set(self):
        ...
        valid_basis = self.data.metadata["basis_set"].lower() in (
            "sto-3g",
            "sto-3g hondo",
        )
        assert valid_basis
