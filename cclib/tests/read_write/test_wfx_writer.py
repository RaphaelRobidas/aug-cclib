...
class WFXTest(unittest.TestCase):
    def test_missing_attribute_error(self):
        ...
        fpath = get_data_path("GAMESS/basicGAMESS-US2017/C_bigbasis.out")
        required_attrs = (
            "atomcoords",
            "atomnos",
            "gbasis",
    ...
    def test_no_of_prims(self):
        ...
        filepaths = [
            get_data_path(f"GAMESS/basicGAMESS-US2017/{fn}") for fn in filenames
        ]
    ...
    def test_mo_normalization_dat(self):
        ...
        fpath = get_data_path("GAMESS/basicGAMESS-US2017/dvb_sp.out")
        data = cclib.io.ccread(fpath)
        wfx = cclib.io.wfxwriter.WFXWriter(data)

        normalized_mocoeffs_wfx = wfx._normalized_mocoeffs()
        datfile = get_data_path("GAMESS/basicGAMESS-US2017/dvb_sp.dat")
    ...
    def test_programs(self):
        ...
        ref_file = "GAMESS/basicGAMESS-US2017/dvb_sp.out"
        programs = {
            "ORCA": "ORCA/basicORCA4.2/dvb_sp.out",
            "NWChem": "NWChem/basicNWChem6.5/dvb_sp_hf.out",
            "Psi4": "Psi4/basicPsi4-1.3.1/dvb_sp_rhf.out",
            "GAMESS-UK": "GAMESS-UK/basicGAMESS-UK8.0/dvb_sp_hf.out",
            "Firefly": "GAMESS/basicFirefly8.0/dvb_sp.out",
        }
        data_ref = cclib.io.ccread(get_data_path(ref_file))
        wfx_ref = cclib.io.wfxwriter.WFXWriter(data_ref)
        norm_mat_ref = wfx_ref._norm_mat()[0]
        mos_ref = wfx_ref._no_of_mos()

        for name, path in programs.items():
            data_prog = cclib.io.ccread(get_data_path(path))
            ...
            assert (
                round(abs(max(np.array(norm_mat_prog) - np.array(norm_mat_ref))), 5)
                == 0
            )
