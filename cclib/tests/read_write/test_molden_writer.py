...
class MOLDENTest(unittest.TestCase):
    def test_missing_attribute_error(self):
        ...
        fpath = get_data_path("GAMESS/basicGAMESS-US2018/dvb_un_sp.out")
    ...
    def test_atoms_section_size_with_ghost(self):
        ...
        fpath = get_data_path("GAMESS/basicGAMESS-US2018/dvb_un_sp_ghost.out")
        data = cclib.io.ccread(fpath)
        writer = cclib.io.moldenwriter.MOLDEN(data)
        writer.ghost = "GH"
    ...
    def test_mo_section_size(self):
        ...
        size_mo_writer = len(
            list(
                filter(
                    None, writer._mo_from_ccdata(mosyms, moenergies, mooccs, mocoeffs)
                )
            )
        )
    ...
    def test_no_section_size(self):
        ...
        fpath = get_data_path("GAMESS/basicGAMESS-US2018/water_cis_dets.out")
        data = cclib.io.ccread(fpath)
        writer = cclib.io.moldenwriter.MOLDEN(data)
        ...
        size_no_writer = len(
            list(
                filter(
                    None, writer._mo_from_ccdata(nosyms, noenergies, nooccs, nocoeffs)
                )
            )
        )
    ...
    def test_molden_cclib_diff(self):
        ...
        for fn in filenames:
            fpath = get_data_path(f"GAMESS/basicGAMESS-US2018/{fn}.out")
            data = cclib.io.ccread(fpath)
            ...
            fpath = get_data_path(f"Molden/molden5.7_{fn}.molden")
