...
from cclib.data.data_handler import get_data, get_data_path
...
class guess_fileypeTest(unittest.TestCase):
    ...
    def test_programs(self):
        ...
        assert (
            self.guess(["Dalton - An Electronic Structure Program"])
            == cclib.parser.DALTON
        )
        ...
        assert (
            self.guess(["Northwest Computational Chemistry Package"])
            == cclib.parser.NWChem
        )
        ...
        assert (
            self.guess(["A Quantum Leap Into The Future Of Chemistry"])
            == cclib.parser.QChem
        )
...
class ccopenTest(unittest.TestCase):
    ...
    def test_list_of_filenames(self):
        ...
        filepaths = [get_data_path(f"Molpro/basicMolpro2012/{fn}") for fn in filenames]
    ...
    def test_cjson_empty_tempfile(self):
        ...
        with tempfile.NamedTemporaryFile() as tf:
            assert isinstance(
                self.ccopen(tf.name, cjson=True), cclib.io.cjsonreader.CJSON
            )
    ...
    def test_url_io(self):
        ...
        for fpath in filepaths:
            assert self.ccopen(get_data_path(fpath), quiet=True).parse().getattributes(
                tolists=True
            ) == self.ccopen(base_url + fpath, quiet=True).parse().getattributes(
                tolists=True
            )
    ...
    def test_multi_url_io(self):
        ...
        base_url = "https://raw.githubusercontent.com/cclib/cclib/master/data/"
        filenames = [
            "Molpro/basicMolpro2012/dvb_gopt.out",
            "Molpro/basicMolpro2012/dvb_gopt.log",
        ]
        assert self.ccopen(
            [get_data_path(fn) for fn in filenames], quiet=True
        ).parse().getattributes(tolists=True) == self.ccopen(
            [base_url + fn for fn in filenames], quiet=True
        ).parse().getattributes(
            tolists=True
        )
    ...
    def test_cjson(self):
        ...
        assert isinstance(
            self.ccopen(StringIO(""), cjson=True), cclib.io.cjsonreader.CJSON
        )

    def test_bz2_io(self):
        """Can we read from a bz2 archive?"""
        file_path = get_data_path("Archives/dvb_gopt.out.bz2")
    ...
    def test_gz_io(self):
        ...
        file_path = get_data_path("Archives/dvb_gopt.out.gz")
        # Test both single-file and multi-file parsing.
        assert self.ccopen(file_path) is not None
        assert self.ccopen([file_path, file_path]) is not None
    ...
    def test_zip_io(self):
        ...
        file_path = get_data_path("Archives/dvb_gopt.out.zip")
        # Test both single-file and multi-file parsing.
        assert self.ccopen(file_path) is not None
        assert self.ccopen([file_path, file_path]) is not None
...
class _determine_output_formatTest(unittest.TestCase):
    ...
    def test_outputclass(self):
        ...
        outputdest = "file.xyz"
        assert (
            self._determine_output_format(outputtype, outputdest)
...
class ccframeTest(unittest.TestCase):
    ...
    def test_ccframe_call_without_pandas(self):
        ...
        with pytest.raises(
            ImportError, match="You must install `pandas` to use this function"
        ):
